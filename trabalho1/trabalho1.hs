...
-- tentar evitar gerar todos os subconjuntos

gera_tabuleiro :: Int -> Int -> [[Int]]
gera_tabuleiro n cont
 | n == cont = [[]]
 | otherwise = [h:t | h<-[1..n], cont < n, t<-(gera_tabuleiro n (cont+1)), (compare_all (h:t) 0 0) == True]
...
--  | i == (length l) - 1 = True
--  | j == (length l) - 1 = compare_all l (i+1) 0
...
nRainhas :: Int -> [[Int]]
nRainhas n = [x | x<-(gera_tabuleiro n 0)]
